...
export ALVIO_BACKEND_API_HOST="${ALVIO_BACKEND_API_HOST:-api_server}"
export ALVIO_WEB_SERVER_HOST="${ALVIO_WEB_SERVER_HOST:-web_server}"

export SSL_CERT_FILE_NAME="${SSL_CERT_FILE_NAME:-ssl.crt}"
export SSL_CERT_KEY_FILE_NAME="${SSL_CERT_KEY_FILE_NAME:-ssl.key}"

echo "Using API server host: $ALVIO_BACKEND_API_HOST"
echo "Using web server host: $ALVIO_WEB_SERVER_HOST"

envsubst '$DOMAIN $SSL_CERT_FILE_NAME $SSL_CERT_KEY_FILE_NAME $ALVIO_BACKEND_API_HOST $ALVIO_WEB_SERVER_HOST' < "/etc/nginx/conf.d/$1" > /etc/nginx/conf.d/app.conf
...
echo
echo "docker logs alvio-api_server-1"
echo

while true; do
  # Use curl to send a request and capture the HTTP status code
  status_code=$(curl -o /dev/null -s -w "%{http_code}\n" "http://${ALVIO_BACKEND_API_HOST}:8080/health")
