...
# Unified config file
CONFIG_FILE="alvio_config.jsonl"
...
if [ -n "$S3_BUCKET_FROM_CONFIG" ]; then
    ...
    S3_BUCKET="${S3_BUCKET:-alvio-ecs-fargate-configs}"
fi

INFRA_ORDER=(
  "alvio_efs_template.yaml"
  "alvio_cluster_template.yaml"
  "alvio_acm_template.yaml"
)

# Deployment order for services
SERVICE_ORDER=(
  "alvio_postgres_service_template.yaml"
  "alvio_redis_service_template.yaml"
  "alvio_vespaengine_service_template.yaml"
  "alvio_model_server_indexing_service_template.yaml"
  "alvio_model_server_inference_service_template.yaml"
  "alvio_backend_api_server_service_template.yaml"
  "alvio_backend_background_server_service_template.yaml"
  "alvio_web_server_service_template.yaml"
  "alvio_nginx_service_template.yaml"
)
...
deploy_infra_stacks() {
    for template_name in "${INFRA_ORDER[@]}"; do
      ...
      if [[ "$template_name" == "alvio_acm_template.yaml" ]]; then
...
echo "Starting deployment of Alvio to ECS Fargate Cluster..."
deploy_infra_stacks
deploy_services_stacks
