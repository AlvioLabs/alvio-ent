...
import { AlvioBotAnalytics, QueryAnalytics, UserAnalytics } from "./usage/types";
...
export const useAlvioBotAnalytics = (timeRange: DateRangePickerValue) => {
  const url = buildApiPath("/api/analytics/admin/alviobot", {
    start: convertDateToStartOfDay(timeRange.from)?.toISOString(),
    end: convertDateToEndOfDay(timeRange.to)?.toISOString(),
  });
  const swrResponse = useSWR<AlvioBotAnalytics[]>(url, errorHandlingFetcher); // TODO

  return {
    ...swrResponse,
    refreshAlvioBotAnalytics: () => mutate(url),
  };
};
