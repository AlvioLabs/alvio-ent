...
from ee.alvio.server.query_history.api import ChatSessionSnapshot
...
def manage_users(alvio_url: str, headers: dict[str, str] | None) -> AllUsersResponse:
    endpoint = alvio_url + "/manage/users"
...
def get_chat_sessions(
    alvio_url: str, headers: dict[str, str] | None, user_id: UUID
) -> ChatSessionsResponse:
    endpoint = alvio_url + "/admin/chat-sessions"
...
def get_session_history(
    alvio_url: str, headers: dict[str, str] | None, session_id: UUID
) -> ChatSessionSnapshot:
    endpoint = alvio_url + f"/admin/chat-session-history/{session_id}"
...
def process_all_chat_feedback(alvio_url: str, api_key: str | None) -> None:
    headers = {"Authorization": f"Bearer {api_key}"} if api_key else None

    all_users = manage_users(alvio_url, headers)
    if not all_users:
...
if __name__ == "__main__":
    ...
    # Or for Alvio MIT Edition API key must be replaced with session cookie
    parser.add_argument(
        "--api-key",
        type=str,
        help="Alvio Admin Level API key",
    )

    args = parser.parse_args()
    process_all_chat_feedback(alvio_url=args.url, api_key=args.api_key)
