...
from alvio.configs.app_configs import REDIS_DB_NUMBER
from alvio.configs.app_configs import REDIS_HOST
from alvio.configs.app_configs import REDIS_PASSWORD
from alvio.configs.app_configs import REDIS_PORT
from alvio.configs.app_configs import REDIS_SSL
...
def alvio_redis(
    ...
    if command == AlvioRedisCommand.purge_connectorsync_taskset:
    ...
    elif command == AlvioRedisCommand.purge_documentset_taskset:
        return purge_by_match_and_type(
            "*documentset_taskset*", "set", batch, dry_run, r
        )
    elif command == AlvioRedisCommand.purge_usergroup_taskset:
        return purge_by_match_and_type("*usergroup_taskset*", "set", batch, dry_run, r)
    elif command == AlvioRedisCommand.purge_locks_blocking_deletion:
        ...
        return 0
    elif command == AlvioRedisCommand.purge_vespa_syncing:
        return purge_by_match_and_type(
            "*connectorsync:vespa_syncing*", "string", batch, dry_run, r
        )
    elif command == AlvioRedisCommand.purge_pidbox:
        return purge_by_match_and_type(
            "*reply.celery.pidbox", "list", batch, dry_run, r
        )
    elif command == AlvioRedisCommand.get_list_element:
        ...
        print(f"{result}")
        return 0
    elif command == AlvioRedisCommand.get_user_token:
        if not user_email:
        ...
        if token_key:
            ...
            return 2
    elif command == AlvioRedisCommand.delete_user_token:
        if not user_email:
        ...
        if delete_user_token_from_redis(r, user_email, dry_run):
            ...
            return 2
    elif command == AlvioRedisCommand.add_invited_user:
        if not user_email:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Alvio Redis Manager")
    parser.add_argument(
        "--command",
        type=AlvioRedisCommand,
        help="The command to run",
        choices=list(AlvioRedisCommand),
        required=True,
    )
