...
from alvio.file_processing.extract_file_text import extract_text_and_images
from alvio.file_processing.extract_file_text import get_file_ext
from alvio.file_processing.extract_file_text import is_accepted_file_ext
from alvio.file_processing.extract_file_text import AlvioExtensionType
...
def _process_file(
    ...
    if not is_accepted_file_ext(extension, AlvioExtensionType.All):
        logger.warning(
    ...
    alvio_metadata, custom_tags = process_alvio_metadata(metadata)
    file_display_name = alvio_metadata.file_display_name or os.path.basename(file_name)
    time_updated = alvio_metadata.doc_updated_at or datetime.now(timezone.utc)
    primary_owners = alvio_metadata.primary_owners
    secondary_owners = alvio_metadata.secondary_owners
    link = alvio_metadata.link
    ...
    # Each file may have file-specific ALVIO_METADATA https://docs.alvio.io/admin/connectors/official/file
    ...
    if extraction_result.metadata:
        ...
        alvio_metadata, more_custom_tags = process_alvio_metadata(
        ...
        source_type = alvio_metadata.source_type or source_type
        primary_owners = alvio_metadata.primary_owners or primary_owners
        secondary_owners = alvio_metadata.secondary_owners or secondary_owners
        time_updated = alvio_metadata.doc_updated_at or time_updated
        file_display_name = alvio_metadata.file_display_name or file_display_name
        title = alvio_metadata.title or alvio_metadata.file_display_name or title
        link = alvio_metadata.link or link
