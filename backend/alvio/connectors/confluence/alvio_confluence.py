...
from alvio.configs.app_configs import CONFLUENCE_CONNECTOR_USER_PROFILES_OVERRIDE
from alvio.configs.app_configs import OAUTH_CONFLUENCE_CLOUD_CLIENT_ID
from alvio.configs.app_configs import OAUTH_CONFLUENCE_CLOUD_CLIENT_SECRET
from alvio.connectors.confluence.models import ConfluenceUser
from alvio.connectors.confluence.user_profile_override import (
    process_confluence_user_profiles_override,
)
from alvio.connectors.confluence.utils import _handle_http_error
from alvio.connectors.confluence.utils import confluence_refresh_tokens
from alvio.connectors.confluence.utils import get_start_param_from_url
...
from alvio.file_processing.html_utils import format_document_soup
        ...
        **kwargs: Any,
        ...
        with self._credentials_provider:
            ...
            if "confluence_refresh_token" in credentials:
                ...
                oauth2_dict: dict[str, Any] = AlvioConfluence._make_oauth2_dict(
                    credentials
                )
                url = (
    ...
    def _initialize_connection_helper(
        ...
        **kwargs: Any,
        ...
        if "confluence_refresh_token" in credentials:
            ...
            oauth2_dict: dict[str, Any] = AlvioConfluence._make_oauth2_dict(credentials)
    ...
    def paginated_cql_user_retrieval(
        ...
        if self._confluence_user_profiles_override:
            ...
            for user_result in self._paginate_url(
                ...
                #         },
                #         '_links': {
                #             'self': 'https://alvioai.atlassian.net/wiki/rest/api/user?accountId=712020:35e60fbb-d0f3-4c91-b8c1-f2dd1d69462d'
                #         }
                #     },
                #     'title': 'Chris Weaver',
                #     'excerpt': '',
...
def get_user_email_from_username__server(
    confluence_client: AlvioConfluence, user_name: str
) -> str | None:
...
def _get_user(confluence_client: AlvioConfluence, user_id: str) -> str:
