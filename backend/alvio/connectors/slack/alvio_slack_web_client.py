...
from alvio.connectors.slack.utils import ALVIO_SLACK_LOCK_BLOCKING_TIMEOUT
from alvio.connectors.slack.utils import ALVIO_SLACK_LOCK_TOTAL_BLOCKING_TIMEOUT
from alvio.connectors.slack.utils import ALVIO_SLACK_LOCK_TTL
from alvio.utils.logger import setup_logger

logger = setup_logger()


class AlvioSlackWebClient(WebClient):
    """Use in combination with the Alvio Retry Handler.
    ...
    def _perform_urllib_http_request(
        ...
        lock: RedisLock = self._redis.lock(
            self._delay_lock,
            timeout=ALVIO_SLACK_LOCK_TTL,
        )

        # try to acquire the lock
        start = time.monotonic()
        while True:
            acquired = lock.acquire(blocking_timeout=ALVIO_SLACK_LOCK_BLOCKING_TIMEOUT)
            if acquired:
                break
            ...
            if time.monotonic() - start > ALVIO_SLACK_LOCK_TOTAL_BLOCKING_TIMEOUT:
                raise RuntimeError(
                    f"AlvioSlackWebClient._perform_urllib_http_request - "
                    f"timed out waiting for lock: {ALVIO_SLACK_LOCK_TOTAL_BLOCKING_TIMEOUT=}"
                )
        ...
        try:
            ...
            if lock.owned():
                ...
                logger.warning(
                    "AlvioSlackWebClient._perform_urllib_http_request lock not owned on release"
                )

        time.monotonic() - start
        # logger.info(
        #     f"AlvioSlackWebClient._perform_urllib_http_request: Releasing lock: {elapsed=}"
        # )
    ...
    def _perform_urllib_http_request_internal(
        ...
        if delay_ms > 0:
            logger.warning(
                f"AlvioSlackWebClient._perform_urllib_http_request_internal delay: "
                f"{delay_ms=} "
                f"{self.num_requests=}"
            )
