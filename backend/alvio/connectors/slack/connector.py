...
from alvio.configs.app_configs import ENABLE_EXPENSIVE_EXPERT_CALLS
from alvio.configs.app_configs import INDEX_BATCH_SIZE
from alvio.configs.app_configs import SLACK_NUM_THREADS
...
from alvio.connectors.slack.access import get_channel_access
from alvio.connectors.slack.models import ChannelType
from alvio.connectors.slack.models import MessageType
from alvio.connectors.slack.models import ThreadType
from alvio.connectors.slack.alvio_retry_handler import AlvioRedisSlackRetryHandler
from alvio.connectors.slack.alvio_slack_web_client import AlvioSlackWebClient
from alvio.connectors.slack.utils import (
    expert_info_from_slack_id,
)
from alvio.connectors.slack.utils import get_message_link
from alvio.connectors.slack.utils import make_paginated_slack_api_call
...
def default_msg_filter(message: MessageType) -> SlackMessageFilterReason | None:
    ...
    if message.get("bot_id") or message.get("app_id"):
        ...
        if bot_profile_name == "AlvioBot Testing":
            return None
...
class SlackConnector(
    ...
    def make_slack_web_client(
        ...
        # for concurrent workers. We've extended it with AlvioRedisSlackRetryHandler.
        ...
        alvio_rate_limit_error_retry_handler = AlvioRedisSlackRetryHandler(
            max_retry_count=max_retry_count,
            delay_key=delay_key,
            r=r,
        )
        custom_retry_handlers: list[RetryHandler] = [
            ...
            alvio_rate_limit_error_retry_handler,
        ]

        client = AlvioSlackWebClient(
...
if __name__ == "__main__":
    ...
    provider = AlvioStaticCredentialsProvider(
