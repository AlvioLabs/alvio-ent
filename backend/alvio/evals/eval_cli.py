...
from alvio.evals.models import EvalationAck
...
def run_remote(
    ...
    Args:
        base_url: Base URL of the remote server (e.g., "https://test.alvio.io")
...
def main() -> None:
    ...
    parser.add_argument(
        "--base-url",
        type=str,
        default="https://test.alvio.io",
        help="Base URL of the remote server (default: https://test.alvio.io)",
    )
    ...
    if args.remote:
        if not args.api_key:
            print("Using API Key from ALVIO_EVAL_API_KEY")
        api_key: str = (
            args.api_key if args.api_key else os.environ.get("ALVIO_EVAL_API_KEY", "")
        )
