...
from alvio.auth.users import current_admin_user
from alvio.auth.users import current_chat_accessible_user
from alvio.auth.users import current_curator_or_admin_user
from alvio.auth.users import current_user
from alvio.background.celery.versioned_apps.client import app as client_app
from alvio.configs.app_configs import DISABLE_AUTH
from alvio.configs.app_configs import ENABLED_CONNECTOR_TYPES
from alvio.configs.app_configs import MOCK_CONNECTOR_FILE_PATH
...
from alvio.configs.constants import ALVIO_METADATA_FILENAME
...
from alvio.connectors.factory import validate_ccpair_for_user
from alvio.connectors.google_utils.google_auth import (
    get_google_oauth_creds,
)
from alvio.connectors.google_utils.google_kv import (
...
from alvio.connectors.google_utils.google_kv import get_auth_url
from alvio.connectors.google_utils.google_kv import (
    get_google_app_cred,
)
from alvio.connectors.google_utils.google_kv import (
...
from alvio.connectors.google_utils.google_kv import verify_csrf
from alvio.connectors.google_utils.shared_constants import DB_CREDENTIALS_DICT_TOKEN_KEY
from alvio.connectors.google_utils.shared_constants import (
    GoogleOAuthAuthenticationMethod,
)
from alvio.db.connector import create_connector
from alvio.db.connector import delete_connector
from alvio.db.connector import fetch_connector_by_id
from alvio.db.connector import fetch_connectors
from alvio.db.connector import get_connector_credential_ids
from alvio.db.connector import mark_ccpair_with_indexing_trigger
from alvio.db.connector import update_connector
from alvio.db.connector_credential_pair import add_credential_to_connector
from alvio.db.connector_credential_pair import (
    fetch_connector_credential_pair_for_connector,
)
from alvio.db.connector_credential_pair import get_cc_pair_groups_for_ids
from alvio.db.connector_credential_pair import get_connector_credential_pair
from alvio.db.connector_credential_pair import get_connector_credential_pairs_for_user
from alvio.db.connector_credential_pair import (
    get_connector_credential_pairs_for_user_parallel,
)
from alvio.db.credentials import cleanup_gmail_credentials
from alvio.db.credentials import cleanup_google_drive_credentials
from alvio.db.credentials import create_credential
from alvio.db.credentials import delete_service_account_credentials
from alvio.db.credentials import fetch_credential_by_id_for_user
from alvio.db.deletion_attempt import check_deletion_attempt_is_allowed
from alvio.db.document import get_document_counts_for_cc_pairs_batched_parallel
from alvio.db.engine.sql_engine import get_session
...
from alvio.db.federated import fetch_all_federated_connectors_parallel
from alvio.db.index_attempt import get_index_attempts_for_cc_pair
from alvio.db.index_attempt import get_latest_index_attempts_by_status
from alvio.db.index_attempt import get_latest_index_attempts_parallel
...
from alvio.db.models import IndexingStatus
from alvio.db.models import User
from alvio.db.models import UserRole
from alvio.file_processing.extract_file_text import extract_file_text
from alvio.file_store.file_store import get_default_file_store
from alvio.file_store.models import ChatFileType
from alvio.key_value_store.interface import KvKeyNotFoundError
from alvio.server.documents.models import AuthStatus
from alvio.server.documents.models import AuthUrl
from alvio.server.documents.models import ConnectorCredentialPairIdentifier
from alvio.server.documents.models import ConnectorIndexingStatusLite
from alvio.server.documents.models import ConnectorIndexingStatusLiteResponse
from alvio.server.documents.models import ConnectorSnapshot
from alvio.server.documents.models import ConnectorStatus
from alvio.server.documents.models import ConnectorUpdateRequest
from alvio.server.documents.models import CredentialBase
from alvio.server.documents.models import CredentialSnapshot
from alvio.server.documents.models import DocsCountOperator
from alvio.server.documents.models import FailedConnectorIndexingStatus
from alvio.server.documents.models import FileUploadResponse
from alvio.server.documents.models import GDriveCallback
from alvio.server.documents.models import GmailCallback
from alvio.server.documents.models import GoogleAppCredentials
from alvio.server.documents.models import GoogleServiceAccountCredentialRequest
from alvio.server.documents.models import GoogleServiceAccountKey
from alvio.server.documents.models import IndexingStatusRequest
from alvio.server.documents.models import ObjectCreationIdResponse
from alvio.server.documents.models import RunConnectorRequest
from alvio.server.documents.models import SourceSummary
from alvio.server.federated.models import FederatedConnectorStatus
from alvio.server.models import StatusResponse
from alvio.server.query_and_chat.chat_utils import mime_type_to_chat_file_type
from alvio.utils.logger import setup_logger
from alvio.utils.telemetry import create_milestone_and_report
from alvio.utils.threadpool_concurrency import CallableProtocol
from alvio.utils.threadpool_concurrency import run_functions_tuples_in_parallel
from alvio.utils.variable_functionality import fetch_ee_implementation_or_noop
...
def extract_zip_metadata(zf: zipfile.ZipFile) -> dict[str, Any]:
    zip_metadata = {}
    try:
        metadata_file_info = zf.getinfo(ALVIO_METADATA_FILENAME)
        with zf.open(metadata_file_info, "r") as metadata_file:
            try:
                ...
                logger.warning(f"Unable to load {ALVIO_METADATA_FILENAME}: {e}")
                # should fail loudly here to let users know that their metadata
                # file is not valid JSON
                raise HTTPException(
                    status_code=400,
                    detail=f"Unable to load {ALVIO_METADATA_FILENAME}: {e}",
                )
    except KeyError:
        logger.info(f"No {ALVIO_METADATA_FILENAME} file")
    return zip_metadata
...
def create_connector_from_model(
    ...
    try:
        ...
        fetch_ee_implementation_or_noop(
            "alvio.db.user_group", "validate_object_creation_for_user", None
        )(
            db_session=db_session,
            user=user,
...
def create_connector_with_mock_credential(
    ...
    try:
        ...
        client_app.send_task(
            AlvioCeleryTask.CHECK_FOR_INDEXING,
            priority=AlvioCeleryPriority.HIGH,
...
def trigger_indexing_for_cc_pair(
    ...
    priority = AlvioCeleryPriority.HIGHEST if is_user_file else AlvioCeleryPriority.HIGH
