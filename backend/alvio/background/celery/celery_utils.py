...
from alvio.configs.app_configs import MAX_PRUNING_DOCUMENT_RETRIEVAL_PER_MINUTE
from alvio.configs.app_configs import VESPA_REQUEST_TIMEOUT
from alvio.connectors.connector_runner import batched_doc_ids
...
def celery_is_listening_to_queue(worker: Any, name: str) -> bool:
    ...
    queue_names = list(worker.io.amqp.queues.consume_from.keys())
...
def make_probe_path(probe: str, hostname: str) -> Path:
    ...
    e.g. /tmp/alvio_k8s_indexing_readiness.txt
    """
    ...
    return Path(f"/tmp/alvio_k8s_{safe_name}_{probe}.txt")
