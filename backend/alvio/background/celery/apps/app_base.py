...
from alvio.background.celery.apps.task_formatters import CeleryTaskColoredFormatter
from alvio.background.celery.apps.task_formatters import CeleryTaskPlainFormatter
from alvio.background.celery.celery_utils import celery_is_worker_primary
from alvio.background.celery.celery_utils import make_probe_path
from alvio.background.celery.tasks.vespa.document_sync import DOCUMENT_SYNC_PREFIX
from alvio.background.celery.tasks.vespa.document_sync import DOCUMENT_SYNC_TASKSET_KEY
from alvio.configs.constants import ALVIO_CLOUD_CELERY_TASK_PREFIX
from alvio.configs.constants import AlvioRedisLocks
from alvio.db.engine.sql_engine import get_sqlalchemy_engine
from alvio.document_index.vespa.shared_utils.utils import wait_for_vespa_with_timeout
...
from alvio.utils.logger import ColoredFormatter
from alvio.utils.logger import LoggerContextVars
from alvio.utils.logger import PlainFormatter
...
def on_task_postrun(
    ...
    **kwds: Any,
    ...
    if task.name.startswith(ALVIO_CLOUD_CELERY_TASK_PREFIX):
...
def on_secondary_worker_init(sender: Any, **kwargs: Any) -> None:
    ...
    while True:
        if r.exists(AlvioRedisLocks.PRIMARY_WORKER):
            break
...
class TenantContextFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if tenant_id:
            # Match the 8 character tenant abbreviation used in AlvioLoggingAdapter
