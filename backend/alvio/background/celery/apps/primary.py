...
from alvio.background.celery.celery_utils import celery_is_worker_primary
from alvio.background.celery.tasks.vespa.document_sync import reset_document_sync
from alvio.configs.app_configs import CELERY_WORKER_PRIMARY_POOL_OVERFLOW
from alvio.configs.constants import CELERY_PRIMARY_WORKER_LOCK_TIMEOUT
from alvio.configs.constants import AlvioRedisConstants
from alvio.configs.constants import AlvioRedisLocks
from alvio.configs.constants import POSTGRES_CELERY_WORKER_PRIMARY_APP_NAME
...
from alvio.redis.redis_connector_stop import RedisConnectorStop
...
celery_app.config_from_object("alvio.background.celery.configs.primary")
...
def on_worker_init(sender: Worker, **kwargs: Any) -> None:
    ...
    r.delete(AlvioRedisLocks.PRIMARY_WORKER)
    ...
    lock: RedisLock = r.lock(
        AlvioRedisLocks.PRIMARY_WORKER,
    ...
    r.delete(AlvioRedisLocks.CHECK_VESPA_SYNC_BEAT_LOCK)

    r.delete(AlvioRedisConstants.ACTIVE_FENCES)
...
celery_app.autodiscover_tasks(
    [
        "alvio.background.celery.tasks.connector_deletion",
        "alvio.background.celery.tasks.docprocessing",
        "alvio.background.celery.tasks.evals",
        "alvio.background.celery.tasks.periodic",
        "alvio.background.celery.tasks.pruning",
        "alvio.background.celery.tasks.shared",
        "alvio.background.celery.tasks.vespa",
        "alvio.background.celery.tasks.llm_model_update",
        "alvio.background.celery.tasks.kg_processing",
    ]
)
