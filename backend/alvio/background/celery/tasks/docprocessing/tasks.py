...
from alvio.background.celery.celery_redis import celery_find_task
from alvio.background.celery.celery_redis import celery_get_unacked_task_ids
from alvio.background.celery.celery_utils import httpx_init_vespa_pool
from alvio.background.celery.tasks.beat_schedule import CLOUD_BEAT_MULTIPLIER_DEFAULT
from alvio.background.celery.tasks.docprocessing.heartbeat import start_heartbeat
from alvio.background.celery.tasks.docprocessing.heartbeat import stop_heartbeat
from alvio.background.celery.tasks.docprocessing.utils import IndexingCallback
from alvio.background.celery.tasks.docprocessing.utils import is_in_repeated_error_state
from alvio.background.celery.tasks.docprocessing.utils import should_index
from alvio.background.celery.tasks.docprocessing.utils import (
    try_creating_docfetching_task,
)
from alvio.background.celery.tasks.models import DocProcessingContext
from alvio.background.indexing.checkpointing_utils import cleanup_checkpoint
from alvio.background.indexing.checkpointing_utils import (
    get_index_attempts_with_old_checkpoints,
)
from alvio.background.indexing.index_attempt_utils import cleanup_index_attempts
from alvio.background.indexing.index_attempt_utils import get_old_index_attempts
from alvio.configs.app_configs import MANAGED_VESPA
from alvio.configs.app_configs import VESPA_CLOUD_CERT_PATH
from alvio.configs.app_configs import VESPA_CLOUD_KEY_PATH
from alvio.configs.constants import CELERY_GENERIC_BEAT_LOCK_TIMEOUT
from alvio.configs.constants import CELERY_INDEXING_LOCK_TIMEOUT
...
from alvio.configs.constants import AlvioRedisConstants
from alvio.configs.constants import AlvioRedisLocks
from alvio.configs.constants import AlvioRedisSignals
from alvio.connectors.models import ConnectorFailure
from alvio.connectors.models import Document
from alvio.connectors.models import IndexAttemptMetadata
from alvio.db.connector import mark_ccpair_with_indexing_trigger
from alvio.db.connector_credential_pair import (
    fetch_indexable_standard_connector_credential_pair_ids,
)
from alvio.db.connector_credential_pair import (
    fetch_indexable_user_file_connector_credential_pair_ids,
)
from alvio.db.connector_credential_pair import get_connector_credential_pair_from_id
from alvio.db.connector_credential_pair import set_cc_pair_repeated_error_state
from alvio.db.engine.sql_engine import get_session_with_current_tenant
from alvio.db.engine.time_utils import get_db_current_time
from alvio.db.enums import ConnectorCredentialPairStatus
from alvio.db.enums import IndexingMode
from alvio.db.enums import IndexingStatus
from alvio.db.index_attempt import create_index_attempt_error
from alvio.db.index_attempt import get_index_attempt
from alvio.db.index_attempt import get_index_attempt_errors_for_cc_pair
from alvio.db.index_attempt import IndexAttemptError
from alvio.db.index_attempt import mark_attempt_canceled
from alvio.db.index_attempt import mark_attempt_failed
from alvio.db.index_attempt import mark_attempt_partially_succeeded
from alvio.db.index_attempt import mark_attempt_succeeded
from alvio.db.indexing_coordination import CoordinationStatus
from alvio.db.indexing_coordination import INDEXING_PROGRESS_TIMEOUT_HOURS
from alvio.db.indexing_coordination import IndexingCoordination
from alvio.db.models import IndexAttempt
from alvio.db.models import SearchSettings
from alvio.db.search_settings import get_current_search_settings
from alvio.db.search_settings import get_secondary_search_settings
from alvio.db.swap_index import check_and_perform_index_swap
from alvio.document_index.factory import get_default_document_index
from alvio.file_store.document_batch_storage import DocumentBatchStorage
from alvio.file_store.document_batch_storage import get_document_batch_storage
from alvio.httpx.httpx_pool import HttpxPool
from alvio.indexing.adapters.document_indexing_adapter import (
    DocumentIndexingBatchAdapter,
)
from alvio.indexing.embedder import DefaultIndexingEmbedder
from alvio.natural_language_processing.search_nlp_models import EmbeddingModel
from alvio.natural_language_processing.search_nlp_models import (
    InformationContentClassificationModel,
)
from alvio.natural_language_processing.search_nlp_models import warm_up_bi_encoder
from alvio.redis.redis_connector import RedisConnector
from alvio.redis.redis_pool import get_redis_client
from alvio.redis.redis_pool import get_redis_replica_client
from alvio.redis.redis_pool import redis_lock_dump
from alvio.redis.redis_pool import SCAN_ITER_COUNT_DEFAULT
from alvio.redis.redis_utils import is_fence
from alvio.server.runtime.alvio_runtime import AlvioRuntime
from alvio.utils.logger import setup_logger
from alvio.utils.middleware import make_randomized_alvio_request_id
...
def _get_fence_validation_block_expiration() -> int:
    ...
    try:
        beat_multiplier = AlvioRuntime.get_beat_multiplier()
    except Exception:
    ...
    with get_session_with_current_tenant() as db_session:
        ...
        if attempt and timed_out:
            if attempt.status == IndexingStatus.IN_PROGRESS:
                ...
                redis_celery = task.io.broker_connection().channel().client  # type: ignore
                task_exists = celery_find_task(
                    attempt.celery_task_id,
                    AlvioCeleryQueues.CONNECTOR_DOC_FETCHING,
                    redis_celery,
                )
                unacked_task_ids = celery_get_unacked_task_ids(
                    AlvioCeleryQueues.CONNECTOR_DOC_FETCHING, redis_celery
                )
...
@shared_task(
    name=AlvioCeleryTask.CHECK_FOR_INDEXING,
    soft_time_limit=300,
    bind=True,
)
def check_for_indexing(self: Task, *, tenant_id: str) -> int | None:
    ...
    the docfetching task (AlvioCeleryTask.CONNECTOR_DOC_FETCHING_TASK).
    ...
    # redis_client_celery: Redis = self.io.broker_connection().channel().client  # type: ignore

    lock_beat: RedisLock = redis_client.lock(
        AlvioRedisLocks.CHECK_INDEXING_BEAT_LOCK,
    ...
    try:
        ...
        if not redis_client.exists(AlvioRedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE):
            ...
            for key_bytes in redis_client_replica.scan_iter(
                ...
                if is_fence(key_bytes) and not redis_client.sismember(
                    AlvioRedisConstants.ACTIVE_FENCES, key_bytes
                ):
                    logger.warning(f"Adding {key_bytes} to the lookup table.")
                    redis_client.sadd(AlvioRedisConstants.ACTIVE_FENCES, key_bytes)

            redis_client.set(
                AlvioRedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE,
                1,
                ex=AlvioRuntime.get_build_fence_lookup_table_interval(),
            )
        ...
        lock_beat.reacquire()
        # we want to run this less frequently than the overall task
        if not redis_client.exists(AlvioRedisSignals.BLOCK_VALIDATE_INDEXING_FENCES):
            ...
            redis_client.set(
                AlvioRedisSignals.BLOCK_VALIDATE_INDEXING_FENCES,
                1,
...
# primary
@shared_task(
    name=AlvioCeleryTask.CHECK_FOR_CHECKPOINT_CLEANUP,
    soft_time_limit=300,
    bind=True,
)
def check_for_checkpoint_cleanup(self: Task, *, tenant_id: str) -> None:
    ...
    lock: RedisLock = redis_client.lock(
        AlvioRedisLocks.CHECK_CHECKPOINT_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for attempt in old_attempts:
                ...
                self.io.send_task(
                    AlvioCeleryTask.CLEANUP_CHECKPOINT,
                    kwargs={
                        "index_attempt_id": attempt.id,
                        "tenant_id": tenant_id,
                    },
                    queue=AlvioCeleryQueues.CHECKPOINT_CLEANUP,
...
# light worker
@shared_task(
    name=AlvioCeleryTask.CLEANUP_CHECKPOINT,
    bind=True,
)
...
# primary
@shared_task(
    name=AlvioCeleryTask.CHECK_FOR_INDEX_ATTEMPT_CLEANUP,
    soft_time_limit=300,
    bind=True,
)
def check_for_index_attempt_cleanup(self: Task, *, tenant_id: str) -> None:
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        AlvioRedisLocks.CHECK_INDEX_ATTEMPT_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for i in range(0, len(old_attempts), batch_size):
                ...
                self.io.send_task(
                    AlvioCeleryTask.CLEANUP_INDEX_ATTEMPT,
                    kwargs={
                        "index_attempt_ids": [attempt.id for attempt in batch],
                        "tenant_id": tenant_id,
                    },
                    queue=AlvioCeleryQueues.INDEX_ATTEMPT_CLEANUP,
                    priority=AlvioCeleryPriority.MEDIUM,
                )
    except Exception:
...
# light worker
@shared_task(
    name=AlvioCeleryTask.CLEANUP_INDEX_ATTEMPT,
    bind=True,
)
...
@shared_task(
    name=AlvioCeleryTask.DOCPROCESSING_TASK,
    bind=True,
)
...
def _docprocessing_task(
    ...
    from alvio.indexing.indexing_pipeline import run_indexing_pipeline
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            index_attempt_metadata = IndexAttemptMetadata(
                ...
                request_id=make_randomized_alvio_request_id("DIP"),
