...
from alvio.background.celery.memory_monitoring import emit_process_memory
...
from alvio.db.engine.sql_engine import get_session_with_shared_schema
...
from alvio.db.models import SyncRecord
from alvio.db.models import UserGroup
from alvio.db.search_settings import get_active_search_settings_list
from alvio.redis.redis_pool import get_redis_client
from alvio.redis.redis_pool import redis_lock_dump
from alvio.utils.logger import is_running_in_container
...
def _collect_queue_metrics(redis_celery: Redis) -> list[Metric]:
    ...
    queue_mappings = {
        ...
        "permissions_sync_queue_length": AlvioCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
        "external_group_sync_queue_length": AlvioCeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC,
        "permissions_upsert_queue_length": AlvioCeleryQueues.DOC_PERMISSIONS_UPSERT,
    }
...
@shared_task(
    name=AlvioCeleryTask.MONITOR_BACKGROUND_PROCESSES,
    ignore_result=True,
    soft_time_limit=_MONITORING_SOFT_TIME_LIMIT,
    time_limit=_MONITORING_TIME_LIMIT,
    queue=AlvioCeleryQueues.MONITORING,
    bind=True,
)
def monitor_background_processes(self: Task, *, tenant_id: str) -> None:
    ...
    lock_monitoring: RedisLock = r.lock(
        AlvioRedisLocks.MONITOR_BACKGROUND_PROCESSES_LOCK,
    ...
    try:
        ...
        redis_celery = self.io.broker_connection().channel().client  # type: ignore
...
@shared_task(
    name=AlvioCeleryTask.CLOUD_MONITOR_ALEMBIC,
)
def cloud_check_alembic() -> bool | None:
    ...
    redis_client = get_redis_client(tenant_id=ALVIO_CLOUD_TENANT_ID)

    lock_beat: RedisLock = redis_client.lock(
        AlvioRedisLocks.CLOUD_CHECK_ALEMBIC_BEAT_LOCK,
...
@shared_task(
    name=AlvioCeleryTask.CLOUD_MONITOR_CELERY_QUEUES, ignore_result=True, bind=True
)
...
@shared_task(name=AlvioCeleryTask.MONITOR_CELERY_QUEUES, ignore_result=True, bind=True)
...
def monitor_celery_queues_helper(
    ...
    r_celery = task.io.broker_connection().channel().client  # type: ignore
    n_celery = celery_get_queue_length("celery", r_celery)
    n_docfetching = celery_get_queue_length(
        AlvioCeleryQueues.CONNECTOR_DOC_FETCHING, r_celery
    )
    n_docprocessing = celery_get_queue_length(AlvioCeleryQueues.DOCPROCESSING, r_celery)
    n_user_files_indexing = celery_get_queue_length(
        AlvioCeleryQueues.USER_FILES_INDEXING, r_celery
    )
    n_user_file_processing = celery_get_queue_length(
        AlvioCeleryQueues.USER_FILE_PROCESSING, r_celery
    )
    n_user_file_project_sync = celery_get_queue_length(
        AlvioCeleryQueues.USER_FILE_PROJECT_SYNC, r_celery
    )
    n_sync = celery_get_queue_length(AlvioCeleryQueues.VESPA_METADATA_SYNC, r_celery)
    n_deletion = celery_get_queue_length(AlvioCeleryQueues.CONNECTOR_DELETION, r_celery)
    n_pruning = celery_get_queue_length(AlvioCeleryQueues.CONNECTOR_PRUNING, r_celery)
    ...
    n_external_group_sync = celery_get_queue_length(
        AlvioCeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC, r_celery
    )
    ...
    n_docprocessing_prefetched = celery_get_unacked_task_ids(
        AlvioCeleryQueues.DOCPROCESSING, r_celery
    )
...
@shared_task(
    name=AlvioCeleryTask.MONITOR_PROCESS_MEMORY,
    ignore_result=True,
    soft_time_limit=_MONITORING_SOFT_TIME_LIMIT,
    time_limit=_MONITORING_TIME_LIMIT,
    queue=AlvioCeleryQueues.MONITORING,
...
@shared_task(
    name=AlvioCeleryTask.CLOUD_MONITOR_CELERY_PIDBOX, ignore_result=True, bind=True
)
