...
from alvio.background.celery.tasks.docprocessing.tasks import ConnectorIndexingLogBuilder
...
from alvio.background.celery.tasks.models import IndexingWatchdogTerminalStatus
from alvio.background.celery.tasks.models import SimpleJobResult
from alvio.background.indexing.job_client import SimpleJob
from alvio.background.indexing.job_client import SimpleJobClient
from alvio.background.indexing.job_client import SimpleJobException
from alvio.background.indexing.run_docfetching import run_docfetching_entrypoint
from alvio.configs.constants import CELERY_INDEXING_WATCHDOG_CONNECTOR_TIMEOUT
...
@shared_task(
    name=AlvioCeleryTask.CONNECTOR_DOC_FETCHING_TASK,
    bind=True,
    acks_late=False,
    track_started=True,
)
def docfetching_proxy_task(
    ...
    1)  determines parameters of the indexing attempt (which connector indexing function to run,
        ...
        connectors are responsible for reading data from an outside source and converting it to Alvio documents.
        At the moment these two steps (reading external data and converting to an Alvio document)
