...
from alvio.db.connector_credential_pair import add_deletion_failure_message
...
from alvio.db.document_set import delete_document_set_cc_pair_relationship__no_commit
...
from alvio.db.index_attempt import delete_index_attempts
from alvio.db.index_attempt import get_recent_attempts_for_cc_pair
from alvio.db.search_settings import get_all_search_settings
...
from alvio.redis.redis_connector_delete import RedisConnectorDelete
from alvio.redis.redis_connector_delete import RedisConnectorDeletePayload
...
@shared_task(
    name=AlvioCeleryTask.CHECK_FOR_CONNECTOR_DELETION,
    ignore_result=True,
...
def check_for_connector_deletion_task(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        AlvioRedisLocks.CHECK_CONNECTOR_DELETION_BEAT_LOCK,
    ...
    try:
        ...
        lock_beat.reacquire()
        if not r.exists(AlvioRedisSignals.BLOCK_VALIDATE_CONNECTOR_DELETION_FENCES):
            ...
            r.set(AlvioRedisSignals.BLOCK_VALIDATE_CONNECTOR_DELETION_FENCES, 1, ex=300)
            ...
            with get_session_with_current_tenant() as db_session:
                ...
                try:
                    try_generate_document_cc_pair_cleanup_tasks(
                        self.io, cc_pair_id, db_session, lock_beat, tenant_id
                    )
                    ...
                    if not redis_connector.stop.fenced:
                        ...
                        revoke_tasks_blocking_deletion(
                            redis_connector, db_session, self.io
                        )
