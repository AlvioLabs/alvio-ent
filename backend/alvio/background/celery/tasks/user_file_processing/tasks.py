...
from alvio.db.models import FileRecord
...
from alvio.indexing.adapters.user_file_indexing_adapter import UserFileIndexingAdapter
...
def _user_file_lock_key(user_file_id: str | UUID) -> str:
    return f"{AlvioRedisLocks.USER_FILE_PROCESSING_LOCK_PREFIX}:{user_file_id}"


def _user_file_project_sync_lock_key(user_file_id: str | UUID) -> str:
    return f"{AlvioRedisLocks.USER_FILE_PROJECT_SYNC_LOCK_PREFIX}:{user_file_id}"


@shared_task(
    name=AlvioCeleryTask.CHECK_FOR_USER_FILE_PROCESSING,
    soft_time_limit=300,
    bind=True,
    ignore_result=True,
)
def check_user_file_processing(self: Task, *, tenant_id: str) -> None:
    ...
    lock: RedisLock = redis_client.lock(
        AlvioRedisLocks.USER_FILE_PROCESSING_BEAT_LOCK,
    ...
    try:
        with get_session_with_current_tenant() as db_session:
            ...
            for user_file_id in user_file_ids:
                self.io.send_task(
                    AlvioCeleryTask.PROCESS_SINGLE_USER_FILE,
                    kwargs={"user_file_id": str(user_file_id), "tenant_id": tenant_id},
                    queue=AlvioCeleryQueues.USER_FILE_PROCESSING,
...
@shared_task(
    name=AlvioCeleryTask.PROCESS_SINGLE_USER_FILE,
    bind=True,
    ignore_result=True,
)
...
@shared_task(
    name=AlvioCeleryTask.CHECK_FOR_USER_FILE_PROJECT_SYNC,
    soft_time_limit=300,
    bind=True,
    ignore_result=True,
)
def check_for_user_file_project_sync(self: Task, *, tenant_id: str) -> None:
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        AlvioRedisLocks.USER_FILE_PROJECT_SYNC_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        with get_session_with_current_tenant() as db_session:
            ...
            for user_file_id in user_file_ids:
                self.io.send_task(
                    AlvioCeleryTask.PROCESS_SINGLE_USER_FILE_PROJECT_SYNC,
                    kwargs={"user_file_id": str(user_file_id), "tenant_id": tenant_id},
                    queue=AlvioCeleryQueues.USER_FILE_PROJECT_SYNC,
                    priority=AlvioCeleryPriority.HIGH,
                )
                enqueued += 1
    finally:
...
@shared_task(
    name=AlvioCeleryTask.PROCESS_SINGLE_USER_FILE_PROJECT_SYNC,
    bind=True,
    ignore_result=True,
)
...
@shared_task(
    name=AlvioCeleryTask.USER_FILE_DOCID_MIGRATION,
    ignore_result=True,
