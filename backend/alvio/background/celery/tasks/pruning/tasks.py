...
from alvio.background.celery.celery_utils import extract_ids_from_runnable_connector
from alvio.background.celery.tasks.beat_schedule import CLOUD_BEAT_MULTIPLIER_DEFAULT
from alvio.background.celery.tasks.docprocessing.utils import IndexingCallbackBase
from alvio.configs.app_configs import ALLOW_SIMULTANEOUS_PRUNING
...
from alvio.db.connector import mark_ccpair_as_pruned
...
from alvio.db.connector_credential_pair import get_connector_credential_pairs
from alvio.db.document import get_documents_for_connector_credential_pair
...
from alvio.db.tag import delete_orphan_tags__no_commit
from alvio.redis.redis_connector import RedisConnector
from alvio.redis.redis_connector_prune import RedisConnectorPrune
from alvio.redis.redis_connector_prune import RedisConnectorPrunePayload
...
from alvio.utils.logger import pruning_ctx
...
@shared_task(
    name=AlvioCeleryTask.CHECK_FOR_PRUNING,
    ignore_result=True,
...
def check_for_pruning(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        AlvioRedisLocks.CHECK_PRUNE_BEAT_LOCK,
    ...
    try:
        ...
        if not r.exists(AlvioRedisSignals.BLOCK_PRUNING):
            ...
            for cc_pair_id in cc_pair_ids:
                ...
                with get_session_with_current_tenant() as db_session:
                    ...
                    payload_id = try_creating_prune_generator_task(
                        self.io, cc_pair, db_session, r, tenant_id
                    )
                    if not payload_id:
            ...
            r.set(AlvioRedisSignals.BLOCK_PRUNING, 1, ex=_get_pruning_block_expiration())

        # we want to run this less frequently than the overall task
        lock_beat.reacquire()
        if not r.exists(AlvioRedisSignals.BLOCK_VALIDATE_PRUNING_FENCES):
            ...
            r.set(
                AlvioRedisSignals.BLOCK_VALIDATE_PRUNING_FENCES,
                1,
...
def try_creating_prune_generator_task(
    ...
    try:
        ...
        result = celery_app.send_task(
            AlvioCeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
            kwargs=dict(
            ...
            queue=AlvioCeleryQueues.CONNECTOR_PRUNING,
            task_id=custom_task_id,
            priority=AlvioCeleryPriority.LOW,
        )
...
@shared_task(
    name=AlvioCeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
    acks_late=False,
    soft_time_limit=JOB_TIMEOUT,
    track_started=True,
    trail=False,
...
def connector_pruning_generator_task(
    ...
    lock: RedisLock = r.lock(
        AlvioRedisLocks.PRUNING_LOCK_PREFIX + f"_{redis_connector.cc_pair_id}",
    ...
    try:
        with get_session_with_current_tenant() as db_session:
            ...
            tasks_generated = redis_connector.prune.generate_tasks(
                set(doc_ids_to_remove), self.io, db_session, None
            )
...
def validate_pruning_fences(
    ...
    queue_len = celery_get_queue_length(AlvioCeleryQueues.CONNECTOR_DELETION, r_celery)
...
def validate_pruning_fence(
    ...
    found = celery_find_task(
        ...
        AlvioCeleryQueues.CONNECTOR_PRUNING,
        r_celery,
    )
    if found:
