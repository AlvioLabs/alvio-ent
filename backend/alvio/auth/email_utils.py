...
from alvio.configs.app_configs import EMAIL_FROM
from alvio.configs.app_configs import SENDGRID_API_KEY
from alvio.configs.app_configs import SMTP_PASS
from alvio.configs.app_configs import SMTP_PORT
from alvio.configs.app_configs import SMTP_SERVER
from alvio.configs.app_configs import SMTP_USER
...
from alvio.configs.constants import ALVIO_SLACK_URL
...
def build_html_email(
    ...
    slack_fragment = ""
    if application_name == ALVIO_DEFAULT_APPLICATION_NAME:
        slack_fragment = f'<br>Have questions? Join our Slack community <a href="{ALVIO_SLACK_URL}">here</a>.'
...
def send_email_with_sendgrid(
    ...
    from_email = Email(mail_from) if mail_from else Email("noreply@alvio.io")
...
def send_email_with_smtplib(
    ...
    msg["Message-ID"] = make_msgid(domain="alvio.io")
...
def send_subscription_cancellation_email(user_email: str) -> None:
    ...
    try:
        load_runtime_settings_fn = fetch_versioned_implementation(
            "alvio.server.enterprise_settings.store", "load_runtime_settings"
        )
    ...
    cta_link = "https://www.alvio.io/pricing"
    ...
    text_content = (
        ...
        "If you change your mind, visit https://www.alvio.io/pricing"
    )
    send_email(
        user_email,
        subject,
