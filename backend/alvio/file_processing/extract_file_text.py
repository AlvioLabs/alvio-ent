...
from alvio.file_processing.file_validation import TEXT_MIME_TYPE
from alvio.file_processing.html_utils import parse_html_page_basic
from alvio.file_processing.unstructured import get_unstructured_api_key
from alvio.file_processing.unstructured import unstructured_to_text
from alvio.utils.file_types import PRESENTATION_MIME_TYPE
from alvio.utils.file_types import WORD_PROCESSING_MIME_TYPE
...
class AlvioExtensionType(IntFlag):
    Plain = auto()
    Document = auto()
    Multimedia = auto()
...
def is_accepted_file_ext(ext: str, ext_type: AlvioExtensionType) -> bool:
    if ext_type & AlvioExtensionType.Plain:
        if ext in ACCEPTED_PLAIN_TEXT_FILE_EXTENSIONS:
            return True

    if ext_type & AlvioExtensionType.Document:
        if ext in ACCEPTED_DOCUMENT_FILE_EXTENSIONS:
            return True

    if ext_type & AlvioExtensionType.Multimedia:
...
def load_files_from_zip(
    ...
    with zipfile.ZipFile(zip_file_io, "r") as zip_file:
        for file_info in zip_file.infolist():
            ...
            ) or file_info.filename == ALVIO_METADATA_FILENAME:
                continue
...
def _extract_alvio_metadata(line: str) -> dict | None:
    """
    Example: first line has:
        <!-- ALVIO_METADATA={"title": "..."} -->
      or
        #ALVIO_METADATA={"title":"..."}
    """
    html_comment_pattern = r"<!--\s*ALVIO_METADATA=\{(.*?)\}\s*-->"
    hashtag_pattern = r"#ALVIO_METADATA=\{(.*?)\}"
...
def read_text_file(
    ...
    encoding: str = "utf-8",
    errors: str = "replace",
    ignore_alvio_metadata: bool = True,
) -> tuple[str, dict]:
    """
    For plain text files. Optionally extracts Alvio metadata from the first line.
    """
    metadata = {}
    file_content_raw = ""
    for ind, line in enumerate(file):
        ...
        if ind == 0 and not ignore_alvio_metadata:
            potential_meta = _extract_alvio_metadata(line)
...
def docx_to_text_and_images(
    ...
    try:
        ...
        text_content_raw, _ = read_text_file(
            file, encoding=encoding, ignore_alvio_metadata=False
        )
