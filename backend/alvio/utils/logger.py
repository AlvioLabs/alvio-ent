...
class AlvioRequestIDFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        from shared_configs.contextvars import ALVIO_REQUEST_ID_CONTEXTVAR

        record.request_id = ALVIO_REQUEST_ID_CONTEXTVAR.get() or "-"
        return True


class AlvioLoggingAdapter(logging.LoggerAdapter):
    def process(
        ...
        fastapi_request_id = ALVIO_REQUEST_ID_CONTEXTVAR.get()
...
def setup_logger(
    ...
    if logger.handlers:
        return AlvioLoggingAdapter(logger, extra=extra)
    ...
    if LOG_FILE_NAME and (is_containerized or DEV_LOGGING_ENABLED):
        ...
        for level in log_levels:
            file_name = (
                f"/var/log/alvio/{LOG_FILE_NAME}_{level}.log"
                if is_containerized
...
def setup_uvicorn_logger(
    ...
    uvicorn_logger.addFilter(AlvioRequestIDFilter())
