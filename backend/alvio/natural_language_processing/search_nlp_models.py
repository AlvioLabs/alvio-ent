...
from alvio.configs.app_configs import INDEXING_EMBEDDING_MODEL_NUM_THREADS
from alvio.configs.app_configs import LARGE_CHUNK_RATIO
from alvio.configs.app_configs import SKIP_WARM_UP
from alvio.configs.model_configs import BATCH_SIZE_ENCODE_CHUNKS
from alvio.configs.model_configs import (
    BATCH_SIZE_ENCODE_CHUNKS_FOR_API_EMBEDDING_SERVICES,
)
from alvio.configs.model_configs import DOC_EMBEDDING_CONTEXT_SIZE
...
from alvio.natural_language_processing.constants import DEFAULT_COHERE_MODEL
from alvio.natural_language_processing.constants import DEFAULT_OPENAI_MODEL
from alvio.natural_language_processing.constants import DEFAULT_VERTEX_MODEL
from alvio.natural_language_processing.constants import DEFAULT_VOYAGE_MODEL
from alvio.natural_language_processing.constants import EmbeddingModelTextType
from alvio.natural_language_processing.exceptions import (
    ModelServerRateLimitError,
)
from alvio.natural_language_processing.utils import get_tokenizer
from alvio.natural_language_processing.utils import tokenizer_trim_content
from alvio.utils.logger import setup_logger
from alvio.utils.search_nlp_models_utils import pass_aws_key
...
WARM_UP_STRINGS = [
    ...
    "https://docs.alvio.io/deployment/getting_started/quickstart",
]
...
class CloudEmbedding:
    ...
    async def _embed_cohere(
        ...
        for text_batch in batch_list(texts, _COHERE_MAX_INPUT_LEN):
            # Does not use the same tokenizer as the Alvio API server but it's approximately the same
...
class EmbeddingModel:
    ...
    def _make_model_server_request(
        ...
        def _make_request() -> Response:
            headers = {}
            if tenant_id:
                headers["X-Alvio-Tenant-ID"] = tenant_id

            if request_id:
                headers["X-Alvio-Request-ID"] = request_id
