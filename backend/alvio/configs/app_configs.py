...
from alvio.configs.constants import DocumentIndexType
from alvio.configs.constants import QueryHistoryType
from alvio.file_processing.enums import HtmlBasedConnectorTransformLinksStrategy
from alvio.prompts.image_analysis import DEFAULT_IMAGE_ANALYSIS_SYSTEM_PROMPT
from alvio.prompts.image_analysis import DEFAULT_IMAGE_SUMMARIZATION_SYSTEM_PROMPT
from alvio.prompts.image_analysis import DEFAULT_IMAGE_SUMMARIZATION_USER_PROMPT
...
# 3. no queries
ALVIO_QUERY_HISTORY_TYPE = QueryHistoryType(
    (os.environ.get("ALVIO_QUERY_HISTORY_TYPE") or QueryHistoryType.NORMAL.value).lower()
)
...
# NOTE: if you are having problems accessing the Alvio web UI locally (especially
...
# restrict access to Alvio to only users with emails from those domains.
# E.g. `VALID_EMAIL_DOMAINS=example.com,example.org` will restrict Alvio
...
# If set, Alvio will listen to the `expires_at` returned by the identity
...
# DB Configs
#####
DOCUMENT_INDEX_NAME = "alvio_index"
...
VESPA_DEPLOYMENT_ZIP = (
    os.environ.get("VESPA_DEPLOYMENT_ZIP") or "/app/alvio/vespa-app.zip"
)
...
# View the list here:
# https://github.com/Alvio-dot-ioAlvioio/blob/main/backenAlviovio/connectors/factory.py
...
# Logs Alvio only model interactions like prompts, responses, messages etc.
LOG_ALVIO_MODEL_INTERACTIONS = (
    os.environ.get("LOG_ALVIO_MODEL_INTERACTIONS", "").lower() == "true"
)
...
BRAINTRUST_PROJECT = os.environ.get("BRAINTRUST_PROJECT", "Alvio")
...
S3_FILE_STORE_BUCKET_NAME = (
    os.environ.get("S3_FILE_STORE_BUCKET_NAME") or "Alvio-file-store-bucket"
)
S3_FILE_STORE_PREFIX = os.environ.get("S3_FILE_STORE_PREFIX") or "Alvio-files"
