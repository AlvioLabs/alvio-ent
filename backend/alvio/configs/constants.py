...
ALVIO_DEFAULT_APPLICATION_NAME = "Alvio"
ALVIO_SLACK_URL = "https://join.slack.com/t/alvio-dot-io/shared_invite/zt-2twesxdr6-5iQitKZQpgq~hYIZ~dv3KA"
SLACK_USER_TOKEN_PREFIX = "xoxp-"
SLACK_BOT_TOKEN_PREFIX = "xoxb-"
ALVIO_EMAILABLE_LOGO_MAX_DIM = 512
...
TENANT_ID_COOKIE_NAME = "alvio_tid"  # tenant id - for workaround cases
ANONYMOUS_USER_COOKIE_NAME = "alvio_anonymous_user"

NO_AUTH_USER_ID = "__no_auth_user__"
NO_AUTH_USER_EMAIL = "anonymous@alvio.io"
...
ALVIO_METADATA_FILENAME = ".alvio_metadata.json"

# Messages
DISABLED_GEN_AI_MSG = (
    "Your System Admin has disabled the Generative AI functionalities of Alvio.\n"
    "Please contact them if you wish to have this enabled.\n"
    "You can still use Alvio as a search engine."
)
...
# API Keys
ALVIO_API_KEY_PREFIX = "API_KEY__"
ALVIO_API_KEY_DUMMY_EMAIL_DOMAIN = "alvioapikey.ai"
...
KV_SETTINGS_KEY = "alvio_settings"
KV_CUSTOMER_UUID_KEY = "customer_uuid"
KV_INSTANCE_DOMAIN_KEY = "instance_domain"
KV_ENTERPRISE_SETTINGS_KEY = "alvio_enterprise_settings"
...
ALVIO_REDIS_FUNCTION_LOCK_PREFIX = "da_function_lock:"

TMP_DRALPHA_PERSONA_NAME = "KG Beta"


class DocumentSource(str, Enum):
    # Special case, document passed in via Alvio APIs without specifying a source type
...
class MilestoneRecordType(str, Enum):
    ...
    CREATED_ALVIO_BOT = "created_alvio_bot"
...
class AlvioRedisConstants:
    ACTIVE_FENCES = "active_fences"


class AlvioCeleryPriority(int, Enum):
    HIGHEST = 0
    HIGH = auto()
    MEDIUM = auto()
    LOW = auto()
...
ALVIO_CLOUD_CELERY_TASK_PREFIX = "cloud"

# the tenant id we use for system level redis operations
ALVIO_CLOUD_TENANT_ID = "cloud"

# the redis namespace for runtime variables
ALVIO_CLOUD_REDIS_RUNTIME = "runtime"
CLOUD_BUILD_FENCE_LOOKUP_TABLE_INTERVAL_DEFAULT = 600


class AlvioCeleryTask:
    DEFAULT = "celery"

    CLOUD_BEAT_TASK_GENERATOR = f"{ALVIO_CLOUD_CELERY_TASK_PREFIX}_generate_beat_tasks"
    CLOUD_MONITOR_ALEMBIC = f"{ALVIO_CLOUD_CELERY_TASK_PREFIX}_monitor_alembic"
    CLOUD_MONITOR_CELERY_QUEUES = (
        f"{ALVIO_CLOUD_CELERY_TASK_PREFIX}_monitor_celery_queues"
    )
    CLOUD_CHECK_AVAILABLE_TENANTS = (
        f"{ALVIO_CLOUD_CELERY_TASK_PREFIX}_check_available_tenants"
    )
    CLOUD_MONITOR_CELERY_PIDBOX = (
        f"{ALVIO_CLOUD_CELERY_TASK_PREFIX}_monitor_celery_pidbox"
    )
...
ALVIO_CELERY_BEAT_HEARTBEAT_KEY = "alvio:celery:beat:heartbeat"
