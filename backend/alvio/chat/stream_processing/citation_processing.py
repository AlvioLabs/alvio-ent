...
from alvio.chat.stream_processing.utils import DocumentIdOrderMapping
from alvio.configs.chat_configs import STOP_STREAM_PAT
from alvio.prompts.constants import TRIPLE_BACKTICK
...
class CitationProcessor:
    ...
    def process_token(
        self, token: str | None
    ) -> Generator[AlvioAnswerPiece | CitationInfo, None, None]:
        # None -> end of stream
        if token is None:
            yield AlvioAnswerPiece(answer_piece=self.curr_segment)
            return
        ...
        if result:
            yield AlvioAnswerPiece(answer_piece=result)
