...
from alvio.chat.models import AlvioAnswer
from alvio.chat.models import AlvioAnswerPiece
from alvio.configs.chat_configs import QUOTE_ALLOWED_ERROR_PERCENT
from alvio.context.search.models import InferenceChunk
from alvio.prompts.constants import ANSWER_PAT
from alvio.prompts.constants import QUOTE_PAT
from alvio.utils.logger import setup_logger
from alvio.utils.text_processing import clean_model_quote
from alvio.utils.text_processing import clean_up_code_blocks
from alvio.utils.text_processing import extract_embedded_json
from alvio.utils.text_processing import shared_precompare_cleanup
...
class AlvioQuotes(BaseModel):
    quotes: list[AlvioQuote]
...
def match_quotes_to_docs(
    ...
    return AlvioQuotes(quotes=alvio_quotes)
...
def _process_answer(
    ...
    if not answer:
        ...
        return AlvioAnswer(answer=None), AlvioQuotes(quotes=[])

    logger.notice(f"Answer: {answer}")
    if not quote_strings:
        logger.debug("No quotes extracted from raw output")
        return AlvioAnswer(answer=answer), AlvioQuotes(quotes=[])
    ...
    return AlvioAnswer(answer=answer), quotes
...
class QuotesProcessor:
    ...
    def process_token(
        self, token: str | None
    ) -> Generator[AlvioAnswerPiece | AlvioQuotes, None, None]:
        # None -> end of stream
        if token is None:
            ...
            if m:
                ...
                for quote_idx in quote_indices:
                    ...
                    if num_backslashes % 2 == 0:
                        yield AlvioAnswerPiece(answer_piece=remaining[:quote_idx])
                        return

                # If no unescaped quote found, yield the remaining string
                if len(remaining) > 0:
                    yield AlvioAnswerPiece(answer_piece=remaining)
                return
            ...
            if self.is_json_prompt and _stream_json_answer_end(model_previous, token):
                ...
                if token:
                    try:
                        ...
                        yield AlvioAnswerPiece(
                            answer_piece=self.hold_quote + token[:answer_token_section]
                        )
                    except ValueError:
                        logger.error("Quotation mark not found in token")
                        yield AlvioAnswerPiece(answer_piece=self.hold_quote + token)
                yield AlvioAnswerPiece(answer_piece=None)
                return
