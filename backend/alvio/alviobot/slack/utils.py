...
from alvio.configs.app_configs import DISABLE_TELEMETRY
from alvio.configs.constants import ID_SEPARATOR
from alvio.configs.constants import MessageType
from alvio.configs.alviobot_configs import ALVIO_BOT_FEEDBACK_VISIBILITY
from alvio.configs.alviobot_configs import ALVIO_BOT_MAX_QPM
from alvio.configs.alviobot_configs import ALVIO_BOT_MAX_WAIT_TIME
from alvio.configs.alviobot_configs import ALVIO_BOT_NUM_RETRIES
from alvio.configs.alviobot_configs import (
    ALVIO_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD,
)
from alvio.configs.alviobot_configs import (
    ALVIO_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS,
)
from alvio.connectors.slack.utils import SlackTextCleaner
...
from alvio.llm.utils import dict_based_prompt_to_langchain_prompt
from alvio.llm.utils import message_to_string
from alvio.alviobot.slack.constants import FeedbackVisibility
from alvio.alviobot.slack.models import ChannelType
from alvio.alviobot.slack.models import ThreadMessage
from alvio.prompts.miscellaneous_prompts import SLACK_LANGUAGE_REPHRASE_PROMPT
...
from alvio.utils.text_processing import replace_whitespaces_w_space
...
_ALVIO_BOT_MESSAGE_COUNT: int = 0
_ALVIO_BOT_COUNT_START_TIME: float = time.time()


def get_alvio_bot_auth_ids(
...
def check_message_limit() -> bool:
    ...
    """
    if ALVIO_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD <= 0:
        return True
    global _ALVIO_BOT_MESSAGE_COUNT
    global _ALVIO_BOT_COUNT_START_TIME
    time_since_start = time.time() - _ALVIO_BOT_COUNT_START_TIME
    if time_since_start > ALVIO_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS:
        _ALVIO_BOT_MESSAGE_COUNT = 0
        _ALVIO_BOT_COUNT_START_TIME = time.time()
    if (_ALVIO_BOT_MESSAGE_COUNT + 1) > ALVIO_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD:
        logger.error(
            f"AlvioBot has reached the message limit {ALVIO_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD}"
            f" for the time period {ALVIO_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS} seconds."
            " These limits are configurable in backend/alvio/configs/alviobot_configs.py"
        )
        return False
    _ALVIO_BOT_MESSAGE_COUNT += 1
    return True
...
def remove_alvio_bot_tag(tenant_id: str, message_str: str, client: WebClient) -> str:
    bot_token_user_id, _ = get_alvio_bot_auth_ids(tenant_id, web_client=client)
...
def _build_error_block(error_message: str) -> Block:
    ...
    """
    display_text = (
        "There was an error displaying all of the Alvio answers."
        f" Please let an admin or an alvio developer know. Error: {error_message}"
    )
    return SectionBlock(text=display_text)


@retry(
    tries=ALVIO_BOT_NUM_RETRIES,
    delay=0.25,
    backoff=2,
    ...
    for reply in replies:
        if "user" in reply and "bot_id" not in reply:
            ...
            if is_alvio_bot_response:
                ...
                if not blocks:
                    logger.warning(f"AlvioBot response has no blocks: {reply}")
                    continue
                ...
                if message is not None and message.startswith("_Filters"):
                    if len(blocks) < 2:
                        ...
                        continue
                    # This is the AlvioBot answer format, if there is a change to how we respond,
                    # this will need to be updated to get the correct "answer" portion
                    message = reply["blocks"][1].get("text", {}).get("text")
            else:
                # Other bots are not counted as the LLM response which only comes from Alvio
                message_type = MessageType.USER
        ...
        message = remove_alvio_bot_tag(tenant_id, message, client=client)
...
def slack_usage_report(action: str, sender_id: str | None, client: WebClient) -> None:
    ...
    if sender_email is not None:
        with get_session_with_current_tenant() as db_session:
            alvio_user = get_user_by_email(email=sender_email, db_session=db_session)

    optional_telemetry(
        record_type=RecordType.USAGE,
        data={"action": action},
        user_id=str(alvio_user.id) if alvio_user else "Non-Alvio-Or-No-Auth-User",
    )


class SlackRateLimiter:
    def __init__(self) -> None:
        self.max_qpm: int | None = ALVIO_BOT_MAX_QPM
        self.max_wait_time = ALVIO_BOT_MAX_WAIT_TIME
...
def get_feedback_visibility() -> FeedbackVisibility:
    try:
        return FeedbackVisibility(ALVIO_BOT_FEEDBACK_VISIBILITY.lower())
    except ValueError:
