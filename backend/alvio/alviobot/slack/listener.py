...
from alvio.configs.app_configs import DEV_MODE
from alvio.configs.app_configs import POD_NAME
from alvio.configs.app_configs import POD_NAMESPACE
from alvio.configs.constants import MessageType
from alvio.configs.constants import AlvioRedisLocks
from alvio.configs.alviobot_configs import NOTIFY_SLACKBOT_NO_ANSWER
from alvio.configs.alviobot_configs import ALVIO_BOT_REPHRASE_MESSAGE
from alvio.configs.alviobot_configs import ALVIO_BOT_RESPOND_EVERY_CHANNEL
from alvio.connectors.slack.utils import expert_info_from_slack_id
from alvio.context.search.retrieval.search_runner import (
    download_nltk_data,
)
...
from alvio.db.engine.tenant_utils import get_all_tenant_ids
from alvio.db.models import SlackBot
from alvio.db.search_settings import get_current_search_settings
from alvio.db.slack_bot import fetch_slack_bot
from alvio.db.slack_bot import fetch_slack_bots
...
from alvio.alviobot.slack.config import get_slack_channel_config_for_bot_and_channel
from alvio.alviobot.slack.config import MAX_TENANTS_PER_POD
from alvio.alviobot.slack.config import TENANT_ACQUISITION_INTERVAL
from alvio.alviobot.slack.config import TENANT_HEARTBEAT_EXPIRATION
from alvio.alviobot.slack.config import TENANT_HEARTBEAT_INTERVAL
from alvio.alviobot.slack.config import TENANT_LOCK_EXPIRATION
from alvio.alviobot.slack.constants import DISLIKE_BLOCK_ACTION_ID
from alvio.alviobot.slack.constants import FEEDBACK_DOC_BUTTON_BLOCK_ACTION_ID
from alvio.alviobot.slack.constants import FOLLOWUP_BUTTON_ACTION_ID
from alvio.alviobot.slack.constants import FOLLOWUP_BUTTON_RESOLVED_ACTION_ID
from alvio.alviobot.slack.constants import GENERATE_ANSWER_BUTTON_ACTION_ID
from alvio.alviobot.slack.constants import IMMEDIATE_RESOLVED_BUTTON_ACTION_ID
from alvio.alviobot.slack.constants import KEEP_TO_YOURSELF_ACTION_ID
from alvio.alviobot.slack.constants import LIKE_BLOCK_ACTION_ID
from alvio.alviobot.slack.constants import SHOW_EVERYONE_ACTION_ID
from alvio.alviobot.slack.constants import VIEW_DOC_FEEDBACK_ID
from alvio.alviobot.slack.handlers.handle_buttons import handle_doc_feedback_button
from alvio.alviobot.slack.handlers.handle_buttons import handle_followup_button
from alvio.alviobot.slack.handlers.handle_buttons import (
...
from alvio.alviobot.slack.handlers.handle_buttons import handle_slack_feedback
from alvio.alviobot.slack.handlers.handle_message import handle_message
from alvio.alviobot.slack.handlers.handle_message import (
    remove_scheduled_feedback_reminder,
)
from alvio.alviobot.slack.handlers.handle_message import schedule_feedback_reminder
from alvio.alviobot.slack.models import SlackContext
from alvio.alviobot.slack.models import SlackMessageInfo
from alvio.alviobot.slack.utils import check_message_limit
from alvio.alviobot.slack.utils import decompose_action_id
from alvio.alviobot.slack.utils import get_channel_name_from_id
from alvio.alviobot.slack.utils import get_channel_type_from_id
from alvio.alviobot.slack.utils import get_alvio_bot_auth_ids
from alvio.alviobot.slack.utils import read_slack_thread
from alvio.alviobot.slack.utils import remove_alvio_bot_tag
from alvio.alviobot.slack.utils import rephrase_slack_message
from alvio.alviobot.slack.utils import respond_in_thread_or_channel
from alvio.alviobot.slack.utils import TenantSocketModeClient
from alvio.redis.redis_pool import get_redis_client
from alvio.server.manage.models import SlackBotTokens
...
class SlackbotHandler:
    ...
    def acquire_tenants(self) -> None:
        ...
        gated_tenants = fetch_ee_implementation_or_noop(
            "alvio.server.tenants.product_gating",
            "get_gated_tenants",
            set(),
        )()
        all_active_tenants = [
        ...
        for tenant_id in all_active_tenants:
            ...
            rlock: RedisLock = redis_client.lock(
                AlvioRedisLocks.SLACK_BOT_LOCK,
    ...
    def send_heartbeats(pod_id: str, tenant_ids: set[str]) -> None:
        ...
        for tenant_id in tenant_ids:
            redis_client = get_redis_client(tenant_id=tenant_id)
            heartbeat_key = f"{AlvioRedisLocks.SLACK_BOT_HEARTBEAT_PREFIX}:{pod_id}"
            redis_client.set(
...
def prefilter_requests(req: SocketModeRequest, client: TenantSocketModeClient) -> bool:
    ...
    bot_token_user_id, bot_token_bot_id = get_alvio_bot_auth_ids(
    ...
    if req.type == "events_api":
        ...
        if (
            ...
            or remove_alvio_bot_tag(tenant_id, msg, client=client.web_client)
        ...
        if event_type == "message":
            ...
            if bot_token_user_id and bot_token_user_id in event_user:
                is_alvio_bot_msg = True

            if bot_token_bot_id and bot_token_bot_id in event_bot_id:
                is_alvio_bot_msg = True

            # AlvioBot should never respond to itself
            if is_alvio_bot_msg:
                logger.info("Ignoring message from AlvioBot (self-message)")
                return False

            # DMs with the bot don't pick up the @AlvioBot so we have to keep the
            # caught events_api
        ...
        if is_bot_message:
            ...
            # If AlvioBot is not specifically tagged and the channel is not set to respond to bots, ignore the message
        ...
        # Can respond in thread if it's an "im" directly to Alvio or @AlvioBot is tagged
        if (
            thread_ts
    ...
    if req.type == "slash_commands":
        ...
        if not channel:
            channel_specific_logger.error(
                "Received AlvioBot command without channel - skipping"
            )
            return False
        ...
        if not sender:
            channel_specific_logger.error(
                "Cannot respond to AlvioBot command without sender to respond to."
            )
            return False
...
def build_request_details(
    ...
    if req.type == "events_api":
        ...
        bot_token_user_id, _ = get_alvio_bot_auth_ids(tenant_id, client.web_client)
        message_ts = event.get("ts")
        thread_ts = event.get("thread_ts")
        ...
        msg = remove_alvio_bot_tag(tenant_id, msg, client=client.web_client)

        if ALVIO_BOT_REPHRASE_MESSAGE:
        ...
        if tagged:
            logger.debug("User tagged AlvioBot")
...
def process_message(
    ...
    client: TenantSocketModeClient,
    respond_every_channel: bool = ALVIO_BOT_RESPOND_EVERY_CHANNEL,
...
def _get_socket_client(
    ...
    # https://docs.alvio.io/admin/getting_started/slack_bot_setup
