...
from alvio.chat.process_message import remove_answer_citations
from alvio.configs.constants import MessageType
from alvio.configs.constants import SearchFeedbackType
from alvio.configs.alviobot_configs import ALVIO_BOT_FOLLOWUP_EMOJI
...
from alvio.alviobot.slack.blocks import build_follow_up_resolved_blocks
from alvio.alviobot.slack.blocks import build_slack_response_blocks
from alvio.alviobot.slack.blocks import get_document_feedback_blocks
...
from alvio.alviobot.slack.handlers.handle_regular_answer import (
    handle_regular_answer,
)
from alvio.alviobot.slack.models import SlackMessageInfo
from alvio.alviobot.slack.utils import build_feedback_id
from alvio.alviobot.slack.utils import decompose_action_id
from alvio.alviobot.slack.utils import fetch_group_ids_from_names
from alvio.alviobot.slack.utils import fetch_slack_user_ids_from_emails
from alvio.alviobot.slack.utils import get_channel_name_from_id
from alvio.alviobot.slack.utils import get_feedback_visibility
...
from alvio.alviobot.slack.utils import update_emote_react
from alvio.server.query_and_chat.models import ChatMessageDetail
...
def handle_publish_ephemeral_message_button(
    ...
    # Obtain alvio_user and chat_message information
    if not chat_message_id:
        raise ValueError("Missing chat_message_id in the payload")

    with get_session_with_current_tenant() as db_session:
        alvio_user = get_user_by_email(user_email, db_session)
        if not alvio_user:
            raise ValueError("Cannot determine alvio_user_id from email in payload")
        try:
            chat_message = get_chat_message(chat_message_id, alvio_user.id, db_session)
        except ValueError:
        ...
        alvio_bot_answer = ChatBasicResponse(
    ...
    if action_id == SHOW_EVERYONE_ACTION_ID:
        ...
        try:
            ...
            respond_in_thread_or_channel(
                ...
                text="Hello! Alvio has some results for you!",
                blocks=all_blocks,
...
def handle_followup_button(
    ...
    update_emote_react(
        emoji=ALVIO_BOT_FOLLOWUP_EMOJI,
        channel=channel_id,
