...
from alvio.chat.chat_utils import prepare_chat_message_request
from alvio.chat.models import ChatBasicResponse
from alvio.chat.process_message import gather_stream
...
from alvio.configs.constants import DEFAULT_PERSONA_ID
from alvio.configs.alviobot_configs import MAX_THREAD_CONTEXT_PERCENTAGE
from alvio.configs.alviobot_configs import ALVIO_BOT_DISABLE_DOCS_ONLY_ANSWER
from alvio.configs.alviobot_configs import ALVIO_BOT_DISPLAY_ERROR_MSGS
from alvio.configs.alviobot_configs import ALVIO_BOT_NUM_RETRIES
from alvio.configs.alviobot_configs import ALVIO_BOT_REACT_EMOJI
from alvio.context.search.enums import OptionalSearchSetting
from alvio.context.search.models import BaseFilters
...
from alvio.db.persona import persona_has_search_tool
from alvio.db.users import get_user_by_email
from alvio.alviobot.slack.blocks import build_slack_response_blocks
from alvio.alviobot.slack.handlers.utils import send_team_member_message
from alvio.alviobot.slack.handlers.utils import slackify_message_thread
...
from alvio.alviobot.slack.utils import SlackRateLimiter
...
from alvio.utils.logger import AlvioLoggingAdapter
...
def handle_regular_answer(
    ...
    client: WebClient,
    channel: str,
    logger: AlvioLoggingAdapter,
    feedback_reminder_id: str | None,
    num_retries: int = ALVIO_BOT_NUM_RETRIES,
    thread_context_percent: float = MAX_THREAD_CONTEXT_PERCENTAGE,
    should_respond_with_error_msgs: bool = ALVIO_BOT_DISPLAY_ERROR_MSGS,
    disable_docs_only_answer: bool = ALVIO_BOT_DISABLE_DOCS_ONLY_ANSWER,
) -> bool:
    ...
    # the message is a dm with the Alvio bot.
    send_as_ephemeral = (
    ...
    # or the message is a dm to the Alvio bot, we should use the proper alvio user from the email.
    # This will make documents privately accessible to the user available to Alvio Bot answers.
    # Otherwise - if not ephemeral or DM to Alvio Bot - we must use None as the user to restrict
    # to public docs.
    ...
    if not message_ts_to_respond_to and not is_slash_command:
        # if the message is not "/alvio" command, then it should have a message ts to respond to
        raise RuntimeError(
    ...
    try:
        ...
        answer = _get_slack_answer(
            ...
            alvio_user=user if can_search_over_private_docs else None,
        )
    ...
    if not answer.answer and disable_docs_only_answer:
        logger.notice(
            ...
            "`ALVIO_BOT_DISABLE_DOCS_ONLY_ANSWER` env variable is set"
        )
        return True
    ...
    try:
        ...
        # For DM (ephemeral message), we need to create a thread via a normal message so the user can see
        # the ephemeral message. This also will give the user a notification which ephemeral message does not.
        # if there is no message_ts_to_respond_to, and we have made it this far, then this is a /alvio message
