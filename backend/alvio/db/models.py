...
from alvio.configs.chat_configs import NUM_POSTPROCESSED_RESULTS
from alvio.configs.constants import (
    DEFAULT_BOOST,
...
from alvio.configs.constants import TokenRateLimitScope
from alvio.connectors.models import InputType
from alvio.db.enums import ChatSessionSharedStatus
...
from alvio.db.pydantic_type import PydanticListType, PydanticType
...
from alvio.kg.models import KGStage
from alvio.server.features.mcp.models import MCPConnectionData
from alvio.utils.encryption import decrypt_bytes_to_string
from alvio.utils.encryption import encrypt_string_to_bytes
from alvio.utils.headers import HeaderItemDict
...
class ConnectorCredentialPair(Base):
    ...
    # {"customer_id": "123567", "company_domain": "@alvio.io"}
...
class ChunkStats(Base):
    ...
    # this should correspond to the ID of the document
    # (as is passed around in Alvio)x
...
class ChatSession(Base):
    ...
    # This chat created by AlvioBot
    alviobot_flow: Mapped[bool] = mapped_column(Boolean, default=False)
...
These models are only used in Enterprise Edition only features in Alvio.
They are kept here to simplify the codebase and avoid having different assumptions
on the shape of data being passed around between the MIT and EE versions of Alvio.

In the MIT version of Alvio, assume these tables are always empty.
"""
...
class User__ExternalUserGroupId(Base):
    ...
    directly adding the Alvio user to the doc ACL list"""
