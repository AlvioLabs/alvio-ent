...
def manage_data_directories(env_name: str, base_path: str, use_cloud_gpu: bool) -> None:
    ...
    directories = {
        "ALVIO_POSTGRES_DATA_DIR": os.path.join(target_path, "postgres/"),
        "ALVIO_VESPA_DATA_DIR": os.path.join(target_path, "vespa/"),
    }
    if not use_cloud_gpu:
        directories["ALVIO_INDEX_MODEL_CACHE_DIR"] = os.path.join(
            target_path, "index_model_cache/"
        )
        directories["ALVIO_INFERENCE_MODEL_CACHE_DIR"] = os.path.join(
...
def start_docker_compose(
    ...
    command = (
        f"docker compose -f docker-compose.search-testing.yml -p alvio-{env_name} up -d"
    )
    command += " --build"
...
if __name__ == "__main__":
    ...
    If no environment_name is indicated, will just clean up all alvio docker containers/volumes/networks
