...
from ee.alvio.background.celery.tasks.external_group_syncing.tasks import (
...
from alvio.db.models import PublicExternalUserGroup
from alvio.db.models import User__ExternalUserGroupId
...
class TestPerformExternalGroupSync:
    ...
    def test_initial_group_sync(self, db_session: Session) -> None:
        ...
        with patch(
            "ee.alvio.background.celery.tasks.external_group_syncing.tasks.get_source_perm_sync_config"
        ) as mock_config:
            # Mock sync config
            ...
            expected_group1_id = build_ext_group_name_for_alvio(
                "group1", DocumentSource.GOOGLE_DRIVE
            )
            expected_group2_id = build_ext_group_name_for_alvio(
                "group2", DocumentSource.GOOGLE_DRIVE
            )
            expected_public_group_id = build_ext_group_name_for_alvio(
    ...
    def test_mixed_regular_and_public_groups(self, db_session: Session) -> None:
        ...
        with patch(
            ...
            # Verify user groups
            user_groups = _get_user_external_groups(db_session, cc_pair.id)
            expected_regular_group_id = build_ext_group_name_for_alvio(
                "regular_group", DocumentSource.GOOGLE_DRIVE
            )
            expected_public_group1_id = build_ext_group_name_for_alvio(
            ...
            expected_public_group2_id = build_ext_group_name_for_alvio(
