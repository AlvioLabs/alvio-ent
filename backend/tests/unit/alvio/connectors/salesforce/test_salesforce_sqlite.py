...
from alvio.connectors.cross_connector_utils.miscellaneous_utils import time_str_to_utc
...
from alvio.connectors.salesforce.doc_conversion import _extract_section
...
from alvio.connectors.salesforce.salesforce_calls import _bulk_retrieve_from_salesforce
from alvio.connectors.salesforce.salesforce_calls import _make_time_filter_for_sf_type
from alvio.connectors.salesforce.salesforce_calls import _make_time_filtered_query
from alvio.connectors.salesforce.salesforce_calls import get_object_by_id_query
...
# from alvio.connectors.salesforce.alvio_salesforce_type import AlvioSalesforceType
# from alvio.connectors.salesforce.salesforce_calls import get_children_of_sf_type
...
def _create_csv_with_example_data(sf_db: AlvioSalesforceSQLite) -> None:
    """
...
def _test_query(sf_db: AlvioSalesforceSQLite) -> None:
    """
...
def _test_upsert(sf_db: AlvioSalesforceSQLite) -> None:
    """
...
def _test_relationships(sf_db: AlvioSalesforceSQLite) -> None:
    """
...
def _test_account_with_children(sf_db: AlvioSalesforceSQLite) -> None:
    """
...
def _test_relationship_updates(sf_db: AlvioSalesforceSQLite) -> None:
    """
...
def _test_get_affected_parent_ids(sf_db: AlvioSalesforceSQLite) -> None:
    """
...
def test_salesforce_sqlite() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        sf_db = AlvioSalesforceSQLite(filename)
        sf_db.connect()
        sf_db.apply_schema()
...
def test_salesforce_bulk_retrieve() -> None:
    ...
    # alvio_sf_type = AlvioSalesforceType("Contact", sf_client)
...
def test_normalize_record() -> None:
    ...
    with open(current_dir / "test_account.csv", "r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            normalized_record, parent_ids = AlvioSalesforceSQLite.normalize_record(row)
...
def test_salesforce_connector_single() -> None:
    ...
    # alvio_parent_sf_type = AlvioSalesforceType(parent_type, sf_client)
    ...
    normalized_record, _ = AlvioSalesforceSQLite.normalize_record(record)
