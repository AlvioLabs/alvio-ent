...
from alvio.connectors.confluence.connector import ConfluenceCheckpoint
...
from tests.unit.alvio.connectors.utils import (
...
def confluence_connector(
    confluence_base_url: str, space_key: str, mock_confluence_client: AlvioConfluence
    ...
    with patch("alvio.connectors.confluence.connector._SLIM_DOC_BATCH_SIZE", 2):
        yield connector
...
def test_load_from_checkpoint_with_page_processing_error(
    ...
    with patch(
        "alvio.connectors.confluence.connector.ConfluenceConnector._convert_page_to_document",
