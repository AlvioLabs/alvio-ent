...
from ee.alvio.db.analytics import fetch_assistant_message_analytics
from ee.alvio.db.analytics import fetch_assistant_unique_users
from ee.alvio.db.analytics import fetch_assistant_unique_users_total
from ee.alvio.db.analytics import fetch_alviobot_analytics
from ee.alvio.db.analytics import fetch_per_user_query_analytics
from ee.alvio.db.analytics import fetch_persona_message_analytics
from ee.alvio.db.analytics import fetch_persona_unique_users
from ee.alvio.db.analytics import fetch_query_analytics
from ee.alvio.db.analytics import user_can_view_assistant_stats
...
class AlviobotAnalyticsResponse(BaseModel):
    total_queries: int
    auto_resolved: int
    date: datetime.date


@router.get("/admin/alviobot")
def get_alviobot_analytics(
    start: datetime.datetime | None = None,
    end: datetime.datetime | None = None,
    _: User | None = Depends(current_admin_user),
    db_session: Session = Depends(get_session),
) -> list[AlviobotAnalyticsResponse]:
    daily_alviobot_info = fetch_alviobot_analytics(
        start=start
        or (
            datetime.datetime.utcnow() - datetime.timedelta(days=_DEFAULT_LOOKBACK_DAYS)
        ),  # default is 30d lookback
