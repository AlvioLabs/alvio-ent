...
from alvio.db.models import ChatMessageFeedback
...
def fetch_alviobot_analytics(
    ...
    Number of AlvioBot Queries (Chat Sessions)
    Number of instances of Negative feedback OR Needing additional help
        (only counting the last feedback)
    """
    # Get every chat session in the time range which is a Alviobot flow
    ...
    subquery_first_ai_response = (
        ...
        .where(
            ...
            ChatSession.alviobot_flow.is_(True),
        )
        .where(
    ...
    results = (
        db_session.query(
            ...
            # feedback aren't counted against Alviobot
            func.sum(
                case(
                    (
                        or_(
        ...
        # message of the chat session where the chat session is Alviobot type and within the time
