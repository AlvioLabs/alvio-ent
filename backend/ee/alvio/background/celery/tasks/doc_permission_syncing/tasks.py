...
from ee.alvio.db.connector_credential_pair import get_all_auto_sync_cc_pairs
from ee.alvio.db.document import upsert_document_external_perms
from ee.alvio.external_permissions.sync_params import get_source_perm_sync_config
...
from alvio.background.celery.celery_redis import celery_get_queued_task_ids
...
from alvio.db.document import get_document_ids_for_connector_credential_pair
from alvio.db.document import get_documents_for_connector_credential_pair_limited_columns
from alvio.db.document import upsert_document_by_connector_credential_pair
...
from alvio.db.users import batch_add_ext_perm_user_if_not_exists
from alvio.db.utils import DocumentRow
from alvio.db.utils import is_retryable_sqlalchemy_error
from alvio.db.utils import SortOrder
...
from alvio.redis.redis_connector_doc_perm_sync import RedisConnectorPermissionSync
from alvio.redis.redis_connector_doc_perm_sync import RedisConnectorPermissionSyncPayload
...
from alvio.server.utils import make_short_id
from alvio.utils.logger import doc_permission_sync_ctx
from alvio.utils.logger import format_error_for_logging
from alvio.utils.logger import LoggerContextVars
...
def _is_external_doc_permissions_sync_due(cc_pair: ConnectorCredentialPair) -> bool:
    ...
    source_sync_period *= int(AlvioRuntime.get_doc_permission_sync_multiplier())
...
@shared_task(
    name=AlvioCeleryTask.CHECK_FOR_DOC_PERMISSIONS_SYNC,
    ignore_result=True,
...
def check_for_doc_permissions_sync(self: Task, *, tenant_id: str) -> bool | None:
    ...
    r_celery: Redis = self.io.broker_connection().channel().client  # type: ignore

    lock_beat: RedisLock = r.lock(
        AlvioRedisLocks.CHECK_CONNECTOR_DOC_PERMISSIONS_SYNC_BEAT_LOCK,
    ...
    try:
        ...
        for cc_pair_id in cc_pair_ids_to_sync:
            payload_id = try_creating_permissions_sync_task(
                self.io, cc_pair_id, r, tenant_id
            )
            if not payload_id:
                continue
        ...
        lock_beat.reacquire()
        if not r.exists(AlvioRedisSignals.BLOCK_VALIDATE_PERMISSION_SYNC_FENCES):
            ...
            r.set(
                AlvioRedisSignals.BLOCK_VALIDATE_PERMISSION_SYNC_FENCES,
                1,
...
def try_creating_permissions_sync_task(
    ...
    try:
        ...
        result = app.send_task(
            AlvioCeleryTask.CONNECTOR_PERMISSION_SYNC_GENERATOR_TASK,
            kwargs=dict(
                cc_pair_id=cc_pair_id,
                tenant_id=tenant_id,
            ),
            queue=AlvioCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
...
@shared_task(
    name=AlvioCeleryTask.CONNECTOR_PERMISSION_SYNC_GENERATOR_TASK,
    acks_late=False,
    soft_time_limit=JOB_TIMEOUT,
    track_started=True,
    trail=False,
...
def connector_permission_sync_generator_task(
    ...
    lock: RedisLock = r.lock(
        AlvioRedisLocks.CONNECTOR_DOC_PERMISSIONS_SYNC_LOCK_PREFIX
...
def validate_permission_sync_fences(
    ...
    queue_len = celery_get_queue_length(
        AlvioCeleryQueues.DOC_PERMISSIONS_UPSERT, r_celery
    )
    ...
    reserved_generator_tasks = celery_get_unacked_task_ids(
        AlvioCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC, r_celery
    )
...
def validate_permission_sync_fence(
    ...
    found = celery_find_task(
        ...
        AlvioCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
        r_celery,
    )
    if found:
