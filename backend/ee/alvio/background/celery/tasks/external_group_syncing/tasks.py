...
from ee.alvio.background.celery.tasks.external_group_syncing.group_sync_utils import (
...
from ee.alvio.db.connector_credential_pair import get_cc_pairs_by_source
from ee.alvio.db.external_perm import ExternalUserGroup
from ee.alvio.db.external_perm import mark_old_external_groups_as_stale
from ee.alvio.db.external_perm import remove_stale_external_groups
from ee.alvio.db.external_perm import upsert_external_groups
from ee.alvio.external_permissions.sync_params import (
...
from alvio.background.error_logging import emit_background_error
from alvio.configs.app_configs import JOB_TIMEOUT
from alvio.configs.constants import CELERY_EXTERNAL_GROUP_SYNC_LOCK_TIMEOUT
...
from alvio.redis.redis_connector_ext_group_sync import RedisConnectorExternalGroupSync
from alvio.redis.redis_connector_ext_group_sync import (
...
@shared_task(
    name=AlvioCeleryTask.CHECK_FOR_EXTERNAL_GROUP_SYNC,
    ignore_result=True,
...
def check_for_external_group_sync(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        AlvioRedisLocks.CHECK_CONNECTOR_EXTERNAL_GROUP_SYNC_BEAT_LOCK,
    ...
    try:
        ...
        lock_beat.reacquire()
        if not r.exists(AlvioRedisSignals.BLOCK_VALIDATE_EXTERNAL_GROUP_SYNC_FENCES):
            # clear fences that don't have associated celery tasks in progress
            # tasks can be in the queue in redis, in reserved tasks (prefetched by the worker),
            # or be currently executing
            try:
                validate_external_group_sync_fences(
                    tenant_id, self.io, r, r_replica, r_celery, lock_beat
                )
            except Exception:
            ...
            r.set(
                AlvioRedisSignals.BLOCK_VALIDATE_EXTERNAL_GROUP_SYNC_FENCES,
                1,
...
def try_creating_external_group_sync_task(
    ...
    try:
        ...
        result = app.send_task(
            AlvioCeleryTask.CONNECTOR_EXTERNAL_GROUP_SYNC_GENERATOR_TASK,
            kwargs=dict(
                cc_pair_id=cc_pair_id,
                tenant_id=tenant_id,
            ),
            queue=AlvioCeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC,
...
@shared_task(
    name=AlvioCeleryTask.CONNECTOR_EXTERNAL_GROUP_SYNC_GENERATOR_TASK,
    acks_late=False,
    soft_time_limit=JOB_TIMEOUT,
    track_started=True,
    trail=False,
...
def connector_external_group_sync_generator_task(
    ...
    lock: RedisLock = r.lock(
        AlvioRedisLocks.CONNECTOR_EXTERNAL_GROUP_SYNC_LOCK_PREFIX
...
def validate_external_group_sync_fence(
    ...
    found = celery_find_task(
        payload.celery_task_id, AlvioCeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC, r_celery
    )
    if found:
