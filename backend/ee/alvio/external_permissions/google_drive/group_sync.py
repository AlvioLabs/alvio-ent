...
from ee.alvio.external_permissions.google_drive.folder_retrieval import (
    get_folder_permissions_by_ids,
)
from ee.alvio.external_permissions.google_drive.folder_retrieval import (
    get_modified_folders,
)
from ee.alvio.external_permissions.google_drive.models import GoogleDrivePermission
from ee.alvio.external_permissions.google_drive.models import PermissionType
...
from alvio.connectors.google_utils.resources import AdminService
...
def _drive_folder_to_alvio_group(
    folder: FolderInfo,
    group_email_to_member_emails_map: dict[str, list[str]],
) -> ExternalUserGroup:
    """
    Converts a folder into an Alvio group.
    """
...
def _drive_member_map_to_alvio_groups(
    drive_id_to_members_map: dict[str, tuple[set[str], set[str]]],
    group_email_to_member_emails_map: dict[str, list[str]],
...
def _google_group_to_alvio_group(
    admin_service: AdminService,
    group_email: str,
) -> ExternalUserGroup:
    """
...
def _build_alvio_groups(
    drive_id_to_members_map: dict[str, tuple[set[str], set[str]]],
    group_email_to_member_emails_map: dict[str, set[str]],
    folder_info: list[FolderInfo],
) -> list[ExternalUserGroup]:
    alvio_groups: list[ExternalUserGroup] = []

    # Convert all drive member definitions to alvio groups
    ...
    # Convert all folder permissions to alvio groups
    ...
    # Convert all group member definitions to alvio groups
...
def gdrive_group_sync(
    ...
    # Each google group is an Alvio group, yield those
    group_email_to_member_emails_map: dict[str, list[str]] = {}
    for group_email in all_group_emails:
        alvio_group = _google_group_to_alvio_group(admin_service, group_email)
        group_email_to_member_emails_map[group_email] = alvio_group.user_emails
        yield alvio_group

    # Each drive is a group, yield those
    for alvio_group in _drive_member_map_to_alvio_groups(
    ...
    for folder in folder_info:
        yield _drive_folder_to_alvio_group(folder, group_email_to_member_emails_map)
